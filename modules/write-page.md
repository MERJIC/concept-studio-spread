...
写入文件后、自检之前，使用当前 Agent 可用的联网搜索或浏览能力核实核心论断。优先查原始论文、大学与研究机构页面、学术出版社等一手或权威来源，不把概念库内部查重当作事实核验。

逐条校验 `scholar_refs`：

1. **身份与年份**：搜索 `{full_cn} {en} {year}`，确认学者身份存在，年份与相关学术贡献相符
2. **论断归因**：用 `{key_claim}` 的关键词查找来源，确认论断确实归于该学者或论文
3. **理论细节**：核实核心机制中的维度、分类和因果描述是否与原理论一致

校验结果处理：

- 全部通过：继续 W5
- 学者不存在或归因错误：修正引用和论断，再重新校验
- 年份或理论细节有偏差：修正具体描述，并记录修正内容
- 搜索不到可靠来源：标记为「未核实」，交付时明确告知用户，不得写成已经证实

同时检查 `modules/scholar-dict.json`。核心机制中出现的学者若不在对照表中，按 `page-spec.md` 的学者名规范新增条目。
